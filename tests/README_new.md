...
1. ✅ `it_dispatches_image_process_job_when_all_jobs_completed` — dispatch когда всё готово
2. ✅ `it_does_not_dispatch_when_metadata_missing` — не dispatch без metadata
3. ✅ `it_does_not_dispatch_when_faces_not_checked` — не dispatch без faces
4. ✅ `it_does_not_dispatch_when_thumbnail_missing` — не dispatch без thumbnail
5. ✅ `it_does_not_dispatch_twice_if_already_processed` — защита от повтора
...
public function it_dispatches_image_process_job_when_all_jobs_completed()
{
    ...
    // Act
    $listener->handle(new ImageJobCompleted($image->id, 'face'));
